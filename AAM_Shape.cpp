                                    ...
                                    double &a, double &b, double &tx, double &ty)const
{
...

    {
        ...
        a	= CV_MAT_ELEM(*Soln, double, 0, 0);
        b	= CV_MAT_ELEM(*Soln, double, 1, 0);
        tx	= CV_MAT_ELEM(*Soln, double, 2, 0);
        ty	= CV_MAT_ELEM(*Soln, double, 3, 0);
    }
...
void AAM_Shape::Sketch(IplImage* image)
{

    for (int i = 0; i < 16; i++)
        cvLine(image, cvPoint(m_vPoint[i].x, m_vPoint[i].y), cvPoint(m_vPoint[i+1].x, m_vPoint[i+1].y), CV_RGB(0,255,0), 2);
    for (int i = 17; i < 26; i++)
        cvLine(image, cvPoint(m_vPoint[i].x, m_vPoint[i].y), cvPoint(m_vPoint[i+1].x, m_vPoint[i+1].y), CV_RGB(0,255,0), 2);
    for (int i = 27; i< 48; i++)
        cvCircle(image, cvPoint(m_vPoint[i].x, m_vPoint[i].y), 1, CV_RGB(0, 255, 0),2);
    for (int i = 48; i < 67; i++)
        cvLine(image, cvPoint(m_vPoint[i].x, m_vPoint[i].y), cvPoint(m_vPoint[i+1].x, m_vPoint[i+1].y), CV_RGB(0,255,0), 2);
